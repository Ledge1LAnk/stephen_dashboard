...
relayLabels.forEach((label, i) => {
  ...
  if (i === 4) {
    ...
    mainPowerRef.on('value', (snapshot) => {
      ...
      // Introduce a small delay to prevent immediate UI reversion due to Firebase listener
      setTimeout(() => {
        if (!isUpdating) {
    ...
    mainPowerCheckbox.addEventListener('change', function() {
      const desiredState = this.checked; // Get the state the user wants to set
      console.log('User attempting to set main power to:', desiredState);

      // Immediately update UI to desired state (optimistic update)
      if (desiredState) {
        div.classList.add('active');
      } else {
        div.classList.remove('active');
      ...
      mainPowerRef.set(desiredState).then(() => {
        console.log('Main power set successfully in Firebase to:', desiredState);
        
        // Control all relays based on main power state, but don't force ON if user has set OFF
        const relayPromises = [];
        for (let j = 1; j <= 4; j++) {
          const relayRef = database.ref(`control/relay${j}`);
          // Only set relay to desiredState if desiredState is ON. If desiredState is OFF, always set to OFF.
          // If desiredState is ON, only set if current Firebase state is also OFF (to avoid overriding user's OFF).
          relayRef.once('value').then(snapshot => {
            const currentRelayState = snapshot.val();
            if (desiredState === false) { // If main power is OFF, turn all relays OFF
              relayPromises.push(relayRef.set(false));
            } else if (desiredState === true && currentRelayState === false) { // If main power is ON, only turn ON relays that are currently OFF
              relayPromises.push(relayRef.set(true));
            }
          });
        }
        ...
        console.log('Relay states updated based on main power toggle.');
      }).catch((error) => {
        console.error('Failed to set main power or relays in Firebase:', error);
    ...
    checkbox.addEventListener('change', function() {
      const desiredState = this.checked; // Get the state the user wants to set
      console.log(`User attempting to set relay ${index} to:`, desiredState);
      ...
      ref.set(desiredState).then(() => {
        console.log(`Relay ${index} set successfully in Firebase to:`, desiredState);
      }).catch((error) => {
        console.error(`Failed to set relay ${index} in Firebase:`, error);
        // Revert UI if Firebase update fails
        this.checked = !desiredState;
        if (this.checked) {
          div.classList.add('active');

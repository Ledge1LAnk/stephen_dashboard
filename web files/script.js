...
relayLabels.forEach((label, i) => {
  ...
  if (i === 4) {
    ...
    mainPowerCheckbox.addEventListener('change', function() {
      if (isUpdating) {
        console.log('Update in progress, ignoring main power toggle');
        return;
      }
      
      const state = this.checked;
      console.log('Setting main power to:', state);
      
      isUpdating = true;
      mainPowerRef.set(state).then(() => {
        console.log('Main power set successfully to:', state);
        
        // Control all relays based on main power state
        const relayPromises = [];
        for (let j = 1; j <= 4; j++) {
          relayPromises.push(database.ref(`control/relay${j}`).set(state));
        }
        
        return Promise.all(relayPromises);
      }).then(() => {
        console.log('All relays set successfully to:', state);
        isUpdating = false;
      }).catch((error) => {
        console.error('Failed to set main power or relays:', error);
        // Revert the checkbox state if Firebase update fails
        this.checked = !state;
        isUpdating = false;
      });
    });
    ...
    checkbox.addEventListener('change', function() {
      if (isUpdating) {
        console.log('Update in progress, ignoring toggle');
        return;
      }
      
      const state = this.checked;
      console.log(`Setting relay ${index} to:`, state);
      
      isUpdating = true;
      ref.set(state).then(() => {
        console.log(`Relay ${index} set successfully to:`, state);
        isUpdating = false;
      }).catch((error) => {
        console.error(`Failed to set relay ${index}:`, error);
        // Revert the checkbox state if Firebase update fails
        this.checked = !state;
        isUpdating = false;
      });
